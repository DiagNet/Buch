...
Der virtuelle Standort wird durch eine FortiGate-Firewall abgesichert, während der physische Standort durch Cisco Geräte mit einer #htl3r.short[zbf] geschützt wird. Die Kommunikation zwischen den beiden Standorten erfolgt über einen verschlüsselten #htl3r.long[site_to_site_vpn] auf Basis von #htl3r.full[ikev2] und #htl3r.full[ipsec].
...
Das erste Subnetz beinhaltet einen Rocky Linux Server, auf dem eine #htl3r.long[nextcloud]-Instanz betrieben wird. Diese dient als Beispiel für einen öffentlich erreichbaren Dienst innerhalb der Infrastruktur. Um den Zugriff aus externen Netzwerken zu ermöglichen, wurde auf der FortiGate-Firewall ein #htl3r.long[port_forwarding] eingerichtet. Dabei wurde ein #htl3r.full[vip]-Objekt verwendet, das eingehende Verbindungen von der externen Adresse auf die interne IP-Adresse des Servers weiterleitet.
...
Der zweite Standort der Testumgebung wird durch zwei physische Cisco Router des Typs ISR4331 abgesichert. Beide Geräte verfügen über eine aktivierte SecurityK9-Lizenz, wodurch erweiterte Sicherheitsfunktionen wie die #htl3r.short[zbf] sowie #htl3r.short[ipsec]-#htl3r.short[vpn] unterstützt werden.

Die beiden Router arbeiten gemeinsam als Gateway für das interne Netzwerk. Zu diesem Zweck wird das #htl3r.short[glbp] eingesetzt. Dadurch stellen beide Geräte eine gemeinsame virtuelle Gateway-Adresse bereit, über die Clients im internen Netzwerk ihr Standard-Gateway erreichen. Neben der Lastverteilung erhöht dieses Verfahren auch die Ausfallsicherheit, da beim Ausfall eines Geräts der verbleibende Router weiterhin als Gateway funktionieren kann.

Auf beiden Routern ist eine #htl3r.short[zbf] implementiert. Netzwerkinterfaces werden dabei bestimmten #htl3r.longpl[sicherheitszone] zugeordnet. Der Datenverkehr zwischen diesen Zonen wird anschließend durch definierte #htl3r.longpl[firewall_policy] kontrolliert.
...
#htl3r.code(
  ...
  ip access-list extended VPN-ESP
    permit esp host 10.61.91.3 host 10.60.90.1
...
Aus diesem Grund wird zusätzlich eine explizite ACL definiert, die #htl3r.short[esp] direkt anhand der IP-Protokollnummer 50 matcht. Die #htl3r.short[acl] `VPN-ESP` stellt somit sicher, dass auch solcher Datenverkehr korrekt der #htl3r.long[class_map] zugeordnet wird und von der Firewall entsprechend verarbeitet werden kann.

=== IKEv2 Site-to-Site VPN

Um eine sichere Kommunikation zwischen den beiden Standorten zu ermöglichen, wird ein verschlüsselter #htl3r.long[site_to_site_vpn] eingesetzt. Dieser verbindet die FortiGate-Firewall des virtuellen Standorts mit dem Cisco Gerät ZBFW-1 des physischen Standorts. Wie bereits erwähnt, basiert der Tunnel auf dem Protokoll #htl3r.short[ikev2] in Kombination mit #htl3r.short[ipsec].
...
AES-256 sorgt für eine starke symmetrische Verschlüsselung des Datenverkehrs, während SHA-256 zur Sicherstellung der Datenintegrität eingesetzt wird. Die #htl3r.full[dh] Gruppe 14 ermöglicht den Austausch der kryptographischen Schlüssel während der Aushandlung. Sie bietet ein höheres Sicherheitsniveau als veraltete Gruppen wie 1, 2 oder 5 und ist dabei weniger Hardware intensiv als die Gruppen 19 oder 21.
...
- 192.168.2.0/24 (#htl3r.long[diagnet] Standort)
- 10.0.10.0/24 (Testnetzwerk)
- 10.0.99.0/24 (Managementnetzwerk)

=== Implementierung auf der FortiGate

Die FortiGate-Firewall verwendet einen route-based VPN. Dabei wird der #htl3r.long[vpn_tunnel] als eigenes logisches Interface im System angelegt. Über statische Routen kann anschließend festgelegt werden, welche Netzwerke über dieses Interface erreichbar sind.

Der folgende Auszug zeigt die Definition der IKE Phase 1:

#htl3r.code(
  caption: [Auszug aus der VPN Konfiguration: `phase1-interface`],
  description: `FortiGate Skript`,
)[
  ```
...
Der Zugriff zwischen dem lokalen Netzwerk und dem #htl3r.long[vpn_tunnel] wird anschließend über #htl3r.longpl[firewall_policy] erlaubt.

=== Implementierung auf dem Cisco Router

Der Cisco Router verwendet eine policy-based VPN-Konfiguration. Dabei wird der Tunnel über eine Crypto Map definiert, welche auf ein Interface angewendet wird. Der zu verschlüsselnde Datenverkehr wird mithilfe einer #htl3r.short[acl] festgelegt.

Die grundlegenden Parameter für #htl3r.short[ikev2] werden im folgenden Beispiel definiert:

#htl3r.code(
  caption: [Auszug aus der VPN Konfiguration: `ikev2 proposal`],
  description: `Cisco IOS Skript`,
)[
  ```
...
Die zu verschlüsselnden Netzwerke werden über eine #htl3r.short[acl] bestimmt:

#htl3r.code(
  caption: [Auszug aus der VPN Konfiguration: `FGT-ACL`],
  description: `Cisco IOS Skript`,
)[
  ```
...
In diesem Kapitel wurde die Absicherung der beiden Standorte durch unterschiedliche Firewall-Technologien beschrieben. Der virtuelle Standort wird durch eine FortiGate-Firewall geschützt, während am physischen Standort zwei Cisco ISR4331 Router mit aktivierter #htl3r.short[zbf] eingesetzt werden. Zwischen beiden Standorten wurde ein #htl3r.short[ikev2]-basierter #htl3r.short[ipsec]-#htl3r.long[vpn_tunnel] aufgebaut, welcher eine verschlüsselte Kommunikation zwischen den jeweiligen Netzsegmenten ermöglicht. Durch definierte #htl3r.long[sicherheitszone]n, #htl3r.longpl[firewall_policy] sowie gezielte Freigaben für den VPN-Traffic wird sichergestellt, dass nur gewünschte Verbindungen zugelassen werden.

Diese Infrastruktur bildet die Grundlage dafür, dass die #htl3r.long[diagnet]-Instanz Geräte am entfernten Standort über den #htl3r.long[vpn_tunnel] erreichen und automatisierte Netzwerktests durchführen kann.
