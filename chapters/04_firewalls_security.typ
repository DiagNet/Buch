...
Der virtuelle Standort wird durch eine FortiGate-Firewall abgesichert, während der physische Standort durch Cisco Geräte mit einer #htl3r.long[zbf] geschützt wird. Die Kommunikation zwischen den beiden Standorten erfolgt über einen verschlüsselten #htl3r.long[site_to_site_vpn] auf Basis von #htl3r.short[ikev2] und #htl3r.short[ipsec].
...
Das erste Subnetz beinhaltet einen Rocky Linux Server, auf dem eine #htl3r.long[nextcloud]-Instanz betrieben wird. Diese dient als Beispiel für einen öffentlich erreichbaren Dienst innerhalb der Infrastruktur. Um den Zugriff aus externen Netzwerken zu ermöglichen, wurde auf der FortiGate-Firewall ein #htl3r.long[port_forwarding] eingerichtet. Dabei wurde ein #htl3r.long[vip]-Objekt verwendet, das eingehende Verbindungen von der externen Adresse auf die interne IP-Adresse des Servers weiterleitet.
...
Die beiden Router arbeiten gemeinsam als Gateway für das interne Netzwerk. Zu diesem Zweck wird das #htl3r.long[glbp] eingesetzt. Dadurch stellen beide Geräte eine gemeinsame virtuelle Gateway-Adresse bereit, über die Clients im internen Netzwerk ihr Standard-Gateway erreichen. Neben der Lastverteilung erhöht dieses Verfahren auch die Ausfallsicherheit, da beim Ausfall eines Geräts der verbleibende Router weiterhin als Gateway funktionieren kann.

Auf beiden Routern ist eine #htl3r.long[zbf] implementiert. Netzwerkinterfaces werden dabei bestimmten #htl3r.longpl[sicherheitszone] zugeordnet. Der Datenverkehr zwischen diesen Zonen wird anschließend durch definierte #htl3r.longpl[firewall_policy] kontrolliert.
...
Diese #htl3r.long[acl] wird anschließend in einer Crypto Map angegeben, welche auf dem externen Interface des Routers aktiviert wird.
